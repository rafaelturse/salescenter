package br.com.salescenter.validator;
...
public class CPFValidator implements Validator {
	...
	public void validate(FacesContext context, UIComponent component, Object value) throws ValidatorException {
		if (!validaCPF(String.valueOf(value))) {
			...
			message.setSummary(BaseCRUD.translateEnumValue("message.invalidCPF"));
			throw new ValidatorException(message);
		}
	}

	private static boolean validaCPF(String cpf) {
		if (cpf == null || cpf.length() != 11 || isCPFPadrao(cpf)) return false;

		try {
			Long.parseLong(cpf);
		} catch (NumberFormatException e) {
			log.severe("Error on method validaCPF");
			e.printStackTrace();
			return false;
		}
